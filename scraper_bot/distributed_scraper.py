...
def run_distributed_scraping(start_id: int, end_id: int):
    logging.info(f"Scraping başlıyor... Veritabanı Oyuncu ID Aralığı: {start_id} - {end_id}")
    
    db = SessionLocalV2()
    players = db.query(Player).filter(Player.id >= start_id, Player.id <= end_id).order_by(Player.id.asc()).all()
...
if __name__ == "__main__":
    ...
    run_distributed_scraping(args.start, args.end)
